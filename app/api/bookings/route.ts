...
export async function GET(request: Request) {
  try {
    ...
    if (statusParam && !BOOKING_STATUSES.includes(statusParam as BookingStatus)) return apiError("Invalid booking status.", 400, "INVALID_FILTER");
    const data = await getBookings(statusParam as BookingStatus | undefined);
    return ok(data, { count: data.length, filters: { status: statusParam ?? null } });
  } catch (error) {
